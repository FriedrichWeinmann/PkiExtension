function Get-PkiCaCertificate {
	<#
	.SYNOPSIS
		Lists certificates from a CA.
	
	.DESCRIPTION
		Lists certificates from a CA.

	.PARAMETER Type
		What kind of certificate to return.
		Defaults to "Issued"

	.PARAMETER Disposition
		The "Type" of a certificate is not _really_ a thing.
		In truth, the CA has two tables - Queue & Log - and different codes for the different states that a certificate in either may be in.
		The views in the MMC console are just an abstraction layer on top of that.

		The Disposition "code" describes what state the certificate is currently in.
	...
	.EXAMPLE
		PS C:\> Get-PkiCaCertificate
	
		Returns all issued certificates from the current computer (assumes localhost is a CA)

    .EXAMPLE
        PS C:\> Get-PkiCaCertificate -FQCAName "ca.contoso.com\MS-CA-01"
	...
	.EXAMPLE
		PS C:\> Get-PkiCaCertificate -Computername ca.contoso.com
...
#>
	...
	[CmdletBinding()]
	param (
		[ValidateSet('Issued', 'Request', 'Revoked', 'Failed', 'Denied')]
		[string]
		$Type = 'Issued',

		[PsfArgumentCompleter({ 'Request', 'Issued', 'Revoked', 'Failed', 'Denied' })]
		[PSFramework.Utility.ScriptTransformation('PkiExtension.Disposition', [int])]
		[int]
		$Disposition,
		...
		[PsfArgumentCompleter('PkiExtension.CertificateProperties')]
		[string[]]
		$Properties,
	...
	begin {
		...
		$data = @{
			FQCAName     = $FQCAName
			Properties   = $propertyHash[$Type]
			Templates    = $templates
			CommonName   = $CommonName
			RequestID    = $RequestID
			Requester    = $Requester
			TemplateName = $TemplateName
			Disposition  = $dispositions[$Type]
		}
		...
		if ($Disposition) {
			...
			if (-not $Properties) {
				switch ($Disposition) {
					9 { $data.Properties = $propertyHash['Request'] }
					20 { $data.Properties = $propertyHash['Issued'] }
					21 { $data.Properties = $propertyHash['Revoked'] }
					30 { $data.Properties = $propertyHash['Failed'] }
					31 { $data.Properties = $propertyHash['Denied'] }
					default { $data.Properties = $propertyHash['Default'] }
				}
			}
		}
	...
	process {
		Invoke-PSFCommand @parameters -ScriptBlock {
			...
			$caView.SetRestriction($caView.GetColumnIndex($false, 'Request Disposition'), $CVR_SEEK_EQ, $CVR_SORT_NONE, $Data.Disposition)
